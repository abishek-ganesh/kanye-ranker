...
(function() {
    // All Kanye quotes in one collection
    const kanyeQuotes = [
        {
    ...
    // Function to set the main quote
    function setMainQuote() {
        const mainQuoteElement = document.querySelector(‘.main-quote’);

        if (mainQuoteElement) {
            const quote = getRandomQuote();
            mainQuoteElement.textContent = `"${quote.quote}"`;
            mainQuoteElement.dataset.quoteType = quote.type;
        }
    }

    // Function to cycle main quote with fade effect
    function cycleMainQuote() {
        const mainQuoteElement = document.querySelector(‘.main-quote’);

        if (mainQuoteElement) {
            mainQuoteElement.style.transition = ‘opacity 0.5s ease’;
    ...
    function init() {
        // Set the main quote on page load
        setMainQuote();
        ...
        setTimeout(() => {
            ...
            setInterval(cycleMainQuote, 10000);
        }, 5000);
    }
